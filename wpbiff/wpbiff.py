...
@click.option('-d', '--date', required=True, help='Pinned date (Format: "YYYY-MM-DD hh:mm")', nargs=2, metavar='DATE')
...
def controller(*args, **kwargs):
    ...
    if kwargs.get('user_agent') is None:
        kwargs['user_agent'] = "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:40.0) Gecko/20100101 Firefox/40.1"

    brute = BruteSession(
                ' '.join(kwargs.get('date')),
                kwargs.get('url'),
                kwargs.get('username'),
                kwargs.get('password'),
                kwargs.get('token'),
                kwargs.get('max_token'),
